...
namespace MedicalPLE.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Aquí validamos si los roles existen sino existen el sistema los crea
                    if (!await _roleManager.RoleExistsAsync(CNT.Admin))
                    {
                        // Los crea tomandolos de la clase de constantes CNT
                        await _roleManager.CreateAsync(new IdentityRole(CNT.Admin));
                        await _roleManager.CreateAsync(new IdentityRole(CNT.Usuario));
                    }

                    //obtener el rol seleccionado, Se accede a radio seleccionado por el administrador, es lo que esta en el formulario
                    ...
                    _logger.LogInformation("Usuario creado con éxito en MedicalPLE.");
